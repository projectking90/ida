...
/**
 * IngredientController 클래스 컨트롤러 클래스 식자재에 관련된 가상 경로를 설정
 * 
 * @author Jo
 */
@Controller
public class IngredientController {
	 ...
	 */
	/**
	 * 식자재 관리 화면을 보여줄 jsp와 가게에 등록된 식자재를 보여주는 메소드 가상주소 /ingredient_form.ida로 접근하면 호출
	 * 
	 * @return mav : /ingredient_form.ida에 맵핑되는 jsp 파일과 가게 식자재 리스트
	 */
	@RequestMapping(value = "/ingredient_form.ida")
	public ModelAndView goIngredientForm(IngredientSearchDTO ingredient_searchDTO, HttpSession session) {
	...
	/**
	 * 식자재 추가 기능 실행시 데이터베이스와 연동 처리할 메소드 가상주소 /ingredient_insert.ida로 접근하면 호출
	 * 
	 * @param IngredientDTO : 식자재 추가를 위해 사용하는 DTO
	 * @return insert_result : 식자재 추가 Query 실행 결과
	 */
	@RequestMapping(value = "/ingredient_insert.ida", method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
	@ResponseBody
	public int insertIngredient(IngredientDTO ingredientDTO, HttpSession session) {
	...
	/**
	 * 식자재 상세보기 화면을 보여줄 jsp와 하나의 식자재 정보를 보여주는 메소드 가상주소 /ingredient_detail_form.ida로
	 * 접근하면 호출
	 * 
	...
	public ModelAndView goIngredientDetailForm(@RequestParam(value = "i_no") int i_no
	// --------------------------------------------------------
	...
	// --------------------------------------------------------
		...
		try {
			session.setAttribute("uri", "ingredient_detail_form.ida");
	...
	public ModelAndView goIngredientUpdateForm(IngredientSearchDTO ingredient_searchDTO, HttpSession session) {
		ModelAndView mav = new ModelAndView();
	...
	/**
	 * 식자재 삭제 화면을 보여줄 jsp와 가게에 등록된 식자재를 보여주는 메소드 가상주소 /ingredient_delete_form.ida로
	 * 접근하면 호출
	 * 
	 * @return mav : /ingredient_delete_form.ida에 맵핑되는 jsp 파일과 가게 식자재 리스트
	 *//*
		 * @RequestMapping(value="/ingredient_delete_form.ida") public ModelAndView
		 * goIngredientDeleteForm( HttpSession session ,IngredientSearchDTO
		 * ingredient_searchDTO) { ModelAndView mav = new ModelAndView();
		 ...
		 * mav.addObject("ingredient_list",ingredient_list);
		 * mav.addObject("ingredient_searchDTO",ingredient_searchDTO); } catch(Exception
		 * e) { // try 구문에서 예외가 발생하면 실행할 구문 설정
	...
	/**
	 * 식자재 삭제 기능 실행 시 데이터베이스와 연동 처리할 메소드 가상주소 /ingredeint_delete.onm로 접근하면 호출
	 * 
	 * @param IngredientDTO : 식자재 삭제를 위해 사용하는 DTO
	 * @return delete_result : 식자제 삭제 Query 실행 결과
	 */
	@RequestMapping(value = "/ingredient_delete.ida", method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
	@ResponseBody
	public int deleteIngredient(IngredientDTO ingredientDTO, IngredientSearchDTO ingredient_searchDTO,
			HttpSession session, @RequestParam(value = "trArr") ArrayList<String> ingredient_delete) {
	...
	/**
	 * 식자재 분석 화면을 보여줄 jsp와 가게에 등록된 식자재를 검색 조건에 따라 보여주는 메소드 가상주소
	 * /ingredient_analysis_form.ida로 접근하면 호출
	 * 
	 * @return mav : /ingredient_analysis_form.ida에 맵핑되는 jsp 파일과 검색 조건에 맞는 가게 식자재
	 *         리스트
	 */
	...
	/**
	 * 식자재 분석 - 차트화면을 보여줄 jsp와 가게에 등록된 식자재를 검색 조건에 따라 차트로 보여주는 메소드 가상주소
	 * /ingredient_analysis_chart_form.ida로 접근하면 호출
	 * 
	 * @return mav : /ingredient_analysis_chart_form.ida에 맵핑되는 jsp 파일과 검색 조건에 맞는 가게
	 *         식자재 차트
	 */
	@RequestMapping(value = "/ingredient_analysis_chart_form.ida")
	public ModelAndView goIngredientAnalysisChartForm(IngredientSearchDTO ingredient_searchDTO, HttpSession session) {
		ModelAndView mav = new ModelAndView();
	...
	/**
	 * 식자재 차트 가상주소 /ingredeint_analysis_chart.ida로 접근하면 호출
	 */
	...
	public ChartDTO getIngredientChartData(ChartSearchDTO chart_searchDTO, HttpSession session,
		...
		try {
			...
			if (chart_search.equals("주")) {
				...
				List<Map<String, String>> week_ingredient_chart = this.ingredientService
						.getWeekIngredientData(chart_searchDTO);
				...
				List<Map<String, String>> month_ingredient_chart = this.ingredientService
						.getMonthIngredientData(chart_searchDTO);
